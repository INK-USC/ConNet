...
class CRF(nn.Module):
    ...
    def forward(self, feats):
        ...
        seq_len, bat_size, _ = feats.size()
        scores = self.hidden2tag(feats).view(-1, 1, self.tagset_size)
        ins_num = scores.size(0)

        if self.snt_emb == 'mean':
            snt_emb = feats.mean(dim = 0)
        elif self.snt_emb == 'hdtl':
            feats = feats.view(seq_len, bat_size, 2, self.hidden_dim // 2)
            snt_emb = torch.cat([feats[-1, :, 0, :], feats[0, :, 1, :]], 1)

        att = torch.matmul(snt_emb, self.attention)
        #print_att = [str(a) for a in att.cpu().detach().numpy()]
        #print('attention:', '\t'.join(print_att))
        ...
        if 'maMulCRFCrowd' in self.task and 'latent' in self.task:
            crowd = torch.matmul(att, self.maMulCRFCrowd.view(self.a_num, -1))
            crowd = crowd.view(1, bat_size, self.tagset_size, self.tagset_size).expand(seq_len, bat_size, self.tagset_size, self.tagset_size).contiguous().view(ins_num, self.tagset_size, self.tagset_size)
            transitions = torch.matmul(transitions, crowd)

        crf_scores = scores.expand(ins_num, self.tagset_size, self.tagset_size) + transitions

        if 'maMulScoreCrowd' in self.task:
            crowd = torch.matmul(att, self.maMulScoreCrowd.view(self.a_num, -1))
            crowd = crowd.view(1, bat_size, self.tagset_size, self.tagset_size).expand(seq_len, bat_size, self.tagset_size, self.tagset_size).contiguous().view(ins_num, self.tagset_size, self.tagset_size)
