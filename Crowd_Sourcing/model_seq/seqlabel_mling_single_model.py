...
class Vanilla_SeqLabel(nn.Module):
    ...
    """
    def __init__(self, c_num_list, c_dim, c_hidden, c_layer, w_num_list, w_dim, w_hidden, w_layer, y_num, droprate, unit='lstm'):
    ...
    def load_pretrained_word_embedding(self, pre_word_embeddings):
        ...
        """
        for i in range(self.lang_num):
            self.word_embed[i].weight = nn.Parameter(pre_word_embeddings[i])
    ...
    def forward(self, f_c, f_p, b_c, b_p, f_w, lang_id):
        """
        ...
        f_c_e = self.drop(self.char_embed[lang_id](f_c))
        b_c_e = self.drop(self.char_embed[lang_id](b_c))
        ...
        w_e = self.word_embed[lang_id](f_w)
