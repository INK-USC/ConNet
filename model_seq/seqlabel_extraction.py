...
from model_seq.crf_extraction import CRF
...
class Vanilla_SeqLabel(nn.Module):
    ...
    """
    def __init__(self, c_num, c_dim, c_hidden, c_layer, w_num, w_dim, w_hidden, w_layer, y_num, droprate, a_num, unit='lstm', task='maMulCrowd', if_bias=True):
        ...
        self.crf = CRF(w_hidden, y_num, self.a_num, self.task)
    ...
    def forward(self, f_c, f_p, b_c, b_p, f_w):
        ...
        crf_outs = self.crf(self.drop(rnn_out)).view(self.a_num, self.word_seq_length, self.batch_size, self.y_num, self.y_num)
