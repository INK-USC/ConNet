...
class eval_batch:
    ...
    def calc_f1_batch(self, decoded_data, target_data, f_w, file_handler):
        ...
        for decoded, target, tokens in zip(batch_decoded, target_data, batch_tokens):
            ...
            print('tokens:', ' '.join([self.gw_map[w] for w in tokens.cpu().numpy()[:length]]))
            print('predts:', ' '.join([self.y_map[t] for t in best_path.cpu().numpy()]))
            print('target:', ' '.join([self.y_map[t] for t in target]))
...
class eval_wc(eval_batch):
    ...
    def calc_score_aggregator(self, tag_model, dataset_loader):
        tag_model.eval()
        self.reset()

        for f_c, f_p, b_c, b_p, f_w, f_y, f_y_m, a_m, g_y in dataset_loader:
            scores = tag_model(f_w)
            #print('####'*20)
