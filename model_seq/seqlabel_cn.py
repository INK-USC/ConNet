...
from model_seq.crf_cn import CRF
...
class Vanilla_SeqLabel(nn.Module):
    ...
    """
    def __init__(self, c_num, c_dim, c_hidden, c_layer, w_num, w_dim, w_hidden, w_layer, y_num, droprate, a_num, unit='lstm', task='maMulCrowd', if_bias=True, test_att=False, train_att=False):
        ...
        self.crf = CRF(w_hidden, y_num, self.a_num, self.task, test_att=test_att, train_att=train_att)
    ...
    def forward(self, f_c, f_p, b_c, b_p, f_w, aid):
        """
        ...
        crf_outs, att_outs = self.crf(self.drop(rnn_out), aid)
        crf_outs = crf_outs.view(self.word_seq_length, self.batch_size, self.y_num, self.y_num)

        return crf_outs, att_outs

    def latent_forward(self, f_c, f_p, b_c, b_p, f_w):
        self.set_batch_seq_size(f_w)
        ...
        crf_outs = self.crf.latent_forward(self.drop(rnn_out)).view(self.word_seq_length, self.batch_size, self.y_num, self.y_num)
