...
if __name__ == "__main__":
    ...
    args.params = args.seq_model + '_unit_' + str(args.seq_rnn_unit) + '_hidim_' + str(args.seq_w_hid) + '_layer_' + str(args.seq_w_layer) + '_drop_' + str(args.seq_droprate) + '_lr_' + str(args.lr) + '_lrDecay_' + str(args.lr_decay) + '_opt_' + str(args.update) + '_epoch_' + str(args.epoch) + '_randseed_' + str(args.rand_seed)
    ...
    try:
        for indexs in range(args.epoch):
            ...
            seq_model.train()
            for f_c, f_p, b_c, b_p, f_w, f_y, f_y_m, g_y in train_dataset.get_tqdm(device):
