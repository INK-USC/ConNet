...
def bea_ent_uns(true, pred, all_labels, all_tags):
    label_le = LabelEncoder().fit(all_labels)
    tag_le = LabelEncoder().fit(all_tags)
    num_classes = len(label_le.classes_)
    num_tags = len(tag_le.classes_)
...
def write_file(truedata, pred_seq, output_file):
    output = open(output_file, 'w')
    with open(truedata) as f:
        i = 0
        for line in f:
            line = line.strip().split()
            if len(line) > 0:
                output.write(' '.join([line[0], line[1], pred_seq[i]]) + '\n')
                i += 1
            else:
                output.write('\n')
...
if __name__ == '__main__':
    all_labels = ['B-MISC', 'I-MISC', 'B-LOC', 'B-ORG', 'B-PER', 'I-LOC', 'I-ORG', 'I-PER', 'O']
    all_tags = ['MISC', 'LOC', 'ORG', 'PER', 'O']
    ...
    truedata = sys.argv[1] #"/home/ron_data/ouyu/data/conll2003/ner/ner-mturk/pruned_ground_truth.txt"
    datapath = sys.argv[2] #"/home/ron_data/ouyu/exp/Vanilla_NER/checkpoint/pruned_mturk/Teachers_maMulScoreCrowd+latent/test_unit_lstm_hidim_300_layer_1_drop_0.5_lr_0.015_lrDecay_0.05_opt_SGD_select_latent_epoch_200_batchSize_32_randseed_999/train_allset_"
    true = read_true(truedata)
    pred = [read_pred(datapath + str(i)) for i in range(47)]
    print(len(true), true[:10])
    print(len(pred), [len(p) for p in pred])
    ...
    #print(pred_seq)
    output_file = datapath + "bea"
    write_file(truedata, pred_seq, output_file)
